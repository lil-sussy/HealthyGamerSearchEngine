...
function VideoResultDisplay({ video }: { video: Video }) {
	const [seekTime, setSeekTime] = useState(video.occurrences[0]?.timestamp ?? 0);
	const [occurrence, setOccurrence] = useState<Occurrence>(video.occurrences[0] ?? { quote: "", url: "", rank: 0, distance: 0, timestamp: 0, duration: 0 });
	...
	useEffect(() => {
		handleSelectTime(video.occurrences[0] ?? { quote: "", url: "", rank: 0, distance: 0, timestamp: 0, duration: 0 });
	}, []);

	return (
		<div className="flex flex-wrap justify-center w-full md:w-3/4 lg:w-1/2 !px-8 !pl-20 !py-4 box-border relative">
			<h1 className="absolute -left-14 top-4 text-shadow-lg !text-[5rem] !font-black text-purple-200">{`#${video.occurrences[0]?.rank ?? 0}`}</h1>
